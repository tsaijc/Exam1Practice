...
def run_test_problem1a():
    ...
    expected = -1.7286  # This is APPROXIMATELY the correct answer.
    answer = problem1a(2, 7)
    print()
    print('Test 3 expected:', expected, '(approximately)')
    print('       actual:  ', answer)

    expected = 0.69237  # This is APPROXIMATELY the correct answer.
    answer = problem1a(10, 14)
    print()
...
def problem1c(n):
    ...
    count = 1
    for k in range(n-1): #starts from 2, so n - 2 and then +1 to get the last number since it starts from 0.
        if is_prime(2+k):
            count = count * (2+k)
